...
class Graph :
    ...
    def djikstra(self,start):
        ...
        #Set number of edges to vertex i as 0 for all
            ...
            for j in self.adj_list[vertex] :
                if self.visited[j[0]]==False:
                    ...
                    if alt_dist < dist_imh:
                        ...
                        self.edges[j[0]] = alt_edges
                    elif alt_dist == dist_imh :
                        if alt_edges < self.edges[j[0]]:
                            self.edges[j[0]] = alt_edges
...
if __name__ == '__main__':
    ...
    if g.dist[end-1] != float("inf"):
        sys.stdout.write(str(g.dist[end-1]) +" "  + str(g.edges[end-1]))
    else :
        print str(-1) + " " + str(-1)